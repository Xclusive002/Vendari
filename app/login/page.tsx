...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError('')
    if (!email.trim()) return setError('Enter the email address on your Vendari account.')
    if (!password) return setError('Enter your password to continue.')
    setLoading(true)
    const result = await login(email.trim(), password)
    if (result.success) router.push('/dashboard')
    else { setError(result.error || 'Check your email and password, then try again.'); setLoading(false) }
  }
