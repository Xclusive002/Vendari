...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-bg text-text-primary">
      ...
      <ScrollReveal><section id="pricing" className="bg-surface px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue">Packages that grow with you</p><h2 className="mt-4 font-display text-3xl font-semibold text-ink sm:text-5xl">Start free. Pay when more clarity pays for itself.</h2></div><Link href="/pricing" className="inline-flex items-center gap-2 text-sm font-semibold text-blue">See all package details <ArrowRight className="h-4 w-4" /></Link></div><div className="mt-12 grid gap-4 lg:grid-cols-3">{[['Free', '₦0', 'Sales, stock, customers, expenses, and simple receipts.'], ['Pro', '₦9,500/mo', 'AI insights, voice entry, advanced reports, forecasting, and payment links.'], ['Growth', '₦25,000/mo', 'Team management, higher capacity, stronger controls, and priority support.']].map(([name, price, copy], index) => <article key={name} className={`rounded-xl border p-6 ${index === 1 ? 'border-blue bg-blue/5 shadow-lg shadow-blue/10' : 'border-border bg-bg'}`}><p className="text-xs font-semibold uppercase tracking-[0.16em] text-blue">{name}</p><p className="mt-4 font-mono text-2xl font-semibold text-ink">{price}</p><p className="mt-3 min-h-[56px] text-sm leading-6 text-text-secondary">{copy}</p><Link href="/pricing" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-ink">Explore this package <ArrowRight className="h-4 w-4 text-blue" /></Link></article>)}</div></div></section></ScrollReveal>
