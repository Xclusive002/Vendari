...
export default function NotificationBell() {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && <div role="dialog" aria-label="Notifications" className="fixed bottom-0 left-0 right-0 z-50 h-full overflow-hidden rounded-t-xl border-t border-border bg-surface sm:absolute sm:bottom-auto sm:left-auto sm:right-0 sm:top-12 sm:h-auto sm:w-[min(22rem,calc(100vw-2rem))] sm:rounded-xl sm:border-t-0 sm:border">
        ...
        <div className="max-h-[calc(100vh-120px)] overflow-y-auto sm:max-h-96">{notifications.length ? notifications.map((notification) => <button key={notification.id} type="button" onClick={() => handleNotificationClick(notification)} className={`flex w-full gap-3 border-b border-border px-4 py-4 text-left transition-colors last:border-0 hover:bg-bg sm:py-3 ${notification.read ? '' : 'bg-blue/5'}`}><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${notification.read ? 'bg-border' : 'bg-brand-gradient'}`} /><span className="min-w-0"><span className="block text-sm font-semibold text-ink">{notification.title}</span><span className="mt-1 block text-sm leading-snug text-text-secondary">{notification.message}</span><span className="mt-2 block text-xs text-text-muted">{relativeTime(notification.created_at)}</span></span></button>) : <p className="px-4 py-8 text-center text-sm text-text-muted">No notifications yet</p>}</div>
      </div>}
    </div>
  )
}
